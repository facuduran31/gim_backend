...
class UsuarioModel {
  ...
  createUsuario = (usuario, callback) => {
    db.query(
      ...
      [usuario.nombre, usuario.apellido, usuario.mail, usuario.password],
      callback
    );
  };
  ...
  updateUsuario = (usuario, callback) => {
    db.query(
      'UPDATE usuario SET nombre = ?, apellido = ?, mail = ? WHERE idUsuario = ?',
      [usuario.nombre, usuario.apellido, usuario.mail, usuario.id],
      callback
    );
  };
