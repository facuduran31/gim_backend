...
class inscripcionModel {
    ...
    getInscripcionById = (idSocioPlan, callback) => {
        db.query('SELECT * FROM socio_plan WHERE idSocioPlan = ? AND deletedAt IS NULL;', [idSocioPlan], callback);
    }


    getInscripcionesByIdGimnasio(idGimnasio, callback) {
        db.query('SELECT * FROM gimnasio g INNER JOIN plan p on g.idGimnasio=p.idGimnasio INNER JOIN socio_plan sp on p.idPlan=sp.idPlan WHERE g.idGimnasio=? AND sp.deletedAt IS NULL;', [idGimnasio], callback);
    }
...
`, [idSocio], callback);
    ...
    deleteInscripcion = (idSocioPlan, callback) => {
        db.query('UPDATE socio_plan SET deletedAt=NOW() WHERE idSocioPlan = ?', [idSocioPlan], callback);

    }

    planActivo = (idSocio, callback) => {
        db.query('SELECT * FROM socio_plan WHERE idSocio=? and fechaFin>CURDATE() AND deletedAt IS NULL;', [idSocio], callback)
    }
