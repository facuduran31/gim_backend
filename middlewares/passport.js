...
passport.use(
  new GoogleStrategy(
    ...
    function (request, accessToken, refreshToken, profile, done) {
      db.query('SELECT * FROM usuario WHERE mail = ?', [profile.email], (err, user) => {
        ...
        if (!err && user.length != 0) {
          //Si no hay error y el usuario ya existe, lo devuelve
          return done(null, user[0]);
        } else {
          const password = uuidv4();
          db.query(
            'INSERT INTO usuario (mail, nombre, apellido, password) VALUES (?, ?, ?, ?)',
            [profile.email, profile.given_name, profile.family_name, password],
            (err, userAdded) => {
